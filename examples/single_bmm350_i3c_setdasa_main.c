/*
 * Single BMM350 EVB + SETDASA example for ESP32-P4
 *
 * This example demonstrates a practical "SETDASA-style" bring-up flow for a
 * single BMM350 connected over I3C:
 *
 *   1. Create I3C bus and attach the device using:
 *        static address  -> dynamic address
 *   2. Probe CHIP_ID over raw I3C
 *   3. Bind Bosch SensorAPI
 *   4. Perform minimal sensor initialization
 *   5. Read magnetic field data continuously
 *
 * Notes:
 * - In ESP-IDF, the practical SETDASA-style flow is implemented through
 *   i3c_master_bus_add_i3c_static_device(), wrapped here by
 *   bmm350_i3c_port_init().
 */
...
static const char *TAG = "BMM350_SETDASA_1X";
...
/* Host-assigned dynamic address for this single-sensor example */
#define BMM350_DYNAMIC_ADDR        0x24
...
void app_main(void)
{
    ESP_LOGI(TAG, "Step 1: Init I3C bus with SETDASA-style attach");

    /*
     * This wraps the practical SETDASA-style flow:
     *   static address -> host-assigned dynamic address
     */
    ...
    ESP_LOGI(TAG,
             "Step 2: Raw I3C probe on assigned dynamic address 0x%02X",
    ...
    ESP_LOGI(TAG, "Step 3: Bind Bosch SensorAPI");
    bmm350_i3c_bind_dev(&g_bmm_dev, &g_bmm_port);

    ESP_LOGI(TAG, "Step 4: Minimal sensor initialization");
    ...
    ESP_LOGI(TAG, "Step 5: Read magnetic field data");
